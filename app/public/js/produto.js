...
addToCartButtons.forEach(button => {
    button.addEventListener('click', () => {
        ...
        // Obtém a quantidade selecionada pelo usuário no input
        const quantityInput = productInfo.querySelector('.botao-quantidade');
        let selectedQuantity = parseInt(quantityInput.value); // Pega a quantidade inserida
        if (isNaN(selectedQuantity) || selectedQuantity < 1) {
            selectedQuantity = 1; // Valor padrão caso seja inválido
        }

        // Verifica se o produto já está no carrinho
        const existingItemIndex = cartItems.findIndex(item => item.name === productName);
        if (existingItemIndex !== -1) {
            // Se o produto já estiver no carrinho, aumenta a quantidade corretamente
...
function showAddedMessage(productName, productImage, totalQuantity) {
...
function updateQuantityInput(increase) {
    const quantityInput = document.querySelector('.botao-quantidade');
    ...
    if (currentQuantity < 1) {
        currentQuantity = 1; // Não deixa diminuir abaixo de 1
    }
...
function addToCart() {
    const productName = document.querySelector('.tittle-p').textContent;
    const productPrice = parseFloat(document.querySelector('.price-p').textContent.replace('R$', '').trim());
    const productImage = document.querySelector('.info-imagem img').src;
    const productQuantity = parseInt(document.querySelector('.botao-quantidade').value);
    ...
    // Atualiza a mensagem com a quantidade total
    showAddedMessage(productName, productImage, productQuantity);

    // Atualiza o resumo do carrinho
    updateCartSummary();
}

// Evento para incrementar e decrementar a quantidade
document.querySelector('.mais').addEventListener('click', () => updateQuantityInput(true));
document.querySelector('.menos').addEventListener('click', () => updateQuantityInput(false));

// Evento de clique no botão "Adicionar"
document.querySelector('.botao-add').addEventListener('click', addToCart);